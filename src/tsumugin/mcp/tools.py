...
#   現在 36: session 系 12 (M4 8 + M6 相同定 2 + Review Queue 2 [#125]) + M11 identify_pattern +
...
#   mem_rietveld_iterate) + operando 診断 4 + M10 anchor 1 + FR-313 discriminate 1 (#130)。
#   アダプタ層 (server.py) が配線に使う単一情報源 🔵 REQ-021。session を取らない計器+アクチュエータ
#   群は rietveld_tools.py / insitu_tools.py / mem_tools.py / operando_diag_tools.py /
#   anchor_tools.py / compare_tools.py / echem_tools.py / interop_tools.py / discriminate_tools.py
#   に分離 (閉ループ丸ごとは出さない = ③ が回す, architecture.md §2/§6)。
