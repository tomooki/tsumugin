...
def _frame_result(
    ...
    # FR-318: alkali 診断 (機能無効なら空 = 既定値のまま)。警告は捨てず sink へ集める
    # (レビュー MEDIUM: M10 経路が lock 完全決定・mult 不一致・infeasible 縮退の必須警告を
    # 全て破棄していた — M9 経路と同じく系列 warnings へ届ける)。
    alkali, warns = alkali_fields(frame.target_composition, charge_constraint, phase_names, res)
