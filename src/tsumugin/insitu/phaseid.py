...
def structure_to_cif(structure: object, path: str | Path) -> str:
...
def identify_new_phases(
    ...
    *,
    ...
    for match in ident.matches:  # score 降順・phase_id 昇順 (identify_phases 保証)
        ...
        try:
            materializer.materialize(ref.phase_id, list(elements), cif_path)
        except Exception:
...
class MPMaterializer:
    ...
    def materialize(self, phase_id: str, elements: Sequence[str], out_path: str) -> str:
        ...
        return structure_to_cif(structure, out_path)
