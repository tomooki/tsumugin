...
def _scaled_chi2_term(metrics: RefinementMetrics) -> tuple[float, float]:
    ...
    【後方互換ガード (s² に対して判定, レビュー対応)】: ``noise_scale`` が ``None`` のときは
      補正なし。それ以外は ``s2 = s*s`` を計算してから ``s2`` の有限性・正値性を検証する
      (``s`` 自体の判定ではない)。極小 s (例 ``noise_scale=1e-200``) は ``s`` 自体は有限かつ
      正だが ``s*s`` が float64 の下限を割ってちょうど 0.0 へアンダーフローしうる。``s`` を
      直接判定すると見逃し、``chi2 / s2`` が ``ZeroDivisionError`` (s2==0.0 の厳密ゼロ除算)
      を送出してしまう (実測)。``s2`` を判定基準にすることでこの経路を確実に捕捉し、
      「s=1 で既定式に厳密一致」の保証を壊れた s 値からも防御的に守る。
