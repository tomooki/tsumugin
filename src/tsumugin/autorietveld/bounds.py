...
def detect_bound_hits(
    ...
    **箱を張った変数が新たに凍結された = 境界に到達した**である。同じ ``parmFrozen`` には
    esd プルーニング (REQ-SAR-103) も書き込むため、まず**箱を張った変数だけに絞る**。

    ただし絞り込みだけでは足りない — 救済プルーニングが凍らせるのは「弱い変数」であって、
    ``0::A0`` / ``:0:Shift`` / ``0:0:Size;i`` のような箱付き変数はどれもその候補になり得る。
    ``dropOOBvars`` は段の**すべての**精密化呼び出し (初回・収束サイクル・救済サイクル) で
    走るので、窓を「1 回の呼び出しの前後」に閉じることはそもそもできない。したがって
    **窓を狭めるのではなく、engine が自分で凍らせた名前を ``frozen_before`` 側へ入れる**
    (`engine._bound_hit_baseline`) ことで帰属を切り分ける — これが engine 側の責務である。
