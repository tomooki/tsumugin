...
- **温度差**: 複数ヒストグラムが異なる測定温度なら各 `HistogramSpec.temperature` を設定
  (静水圧歪み Dij で格子差を吸収)。
- **ADP (Uiso) の解放範囲**は 3 通りで、`None` と `()` は**別の意味**である (#189/#211):
  `free_uiso_labels=None` (既定) = 未指定 → uiso 段で全原子解放 / `free_uiso_labels=()` =
  **その相を丸ごと凍結** / 非空 = 列挙した原子だけ解放。相の一部だけ凍結したいなら
  `frozen_uiso_labels` (`frozen_coord_labels` の ADP 版、解放集合から差し引く)。
