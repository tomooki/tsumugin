...
### W2 T4 の TOF を詰める (#179) — **一区切り (2026-08-20): 68.61% → 19.25%**

| # | 変更 | 総合 | 11BM | PG3-1066 | PG3-2665 |
|---|---|---|---|---|---|
| 0 | 同一条件の初回 | 68.61% | 72.99 | 49.06 | 48.89 |
| 1 | TOF の α/β を装置ファイルから写す (`TOF_Exponential`) | 45.46% | 44.58 | 48.18 | 48.98 |
| 2 | `tof_profile` 段を X 線プロファイルの**前**に置く | 67.58% | 73.03 | 36.70 | 44.75 |
| 3 | 同じ段を**後ろ**へ移す | 43.49% | 43.94 | 33.95 | 47.17 |
| 4 | 背景 20 項 | 26.31% | 21.19 | 32.71 | 45.05 |
| 5 | + 放射光のプロファイル種付け (`seed_profile`) | **19.25%** | **8.67** | 29.01 | 44.57 |

- [x] TOF ピーク形状: **α/β は写せる** (幅の sig-1/sig-2 と違い関数形が解ける)。
      `exp_conv_const = lr·difC/(a0 + a1/d^wexp)` は**時間定数**なので GSAS の速度定数の逆数。
- [x] `tof_profile` 段を既定レシピへ。**置き場所が判定を通じて結合している** — 総合 Rwp で
      受理を決めるので、放射光が外れたまま TOF を解放すると後段の X 線プロファイルが
      revert される (#2 と #3 の差 24 ポイント)。
- [x] `seed_profile` を engine → ② `auto_rietveld(seed_profile=)` → ③ skill まで配線。
      **放射源で効果が逆転する** (放射光 43.9→8.7% / CW 中性子 5.54→9.76% で validity fail)
      ので既定 OFF。`backend="gsasii"` に渡すと**黙って無視せずエラー**。
- [x] 背景項数: 20 で 43.5→26.3%、**24 で 67.4% へ跳ねる** (非単調)。恒久ガードは
      `test_benchmark_t4_tuned_configuration`。
- [ ] 背景項数の**per-histogram 化**: 11BM は 20 項・TOF は 6 項が妥当だが、
      `background_coeffs` は全ヒストグラム共通の int。`HistogramSpec` に持たせるのが正しい形
      だが **GSAS 経路も同時に実装しないと「片方で黙って無視される引数」**になるため、
      別 Issue に切り出す (W2 では入れない)。
- [ ] `size_strain` 段で tc.exe が異常終了する (両条件で再現)。TOF の粒径/微小歪みを
      一切モデルに入れられていない。**次の一手はこれ**。
- [ ] CaF2 の Uiso < 0 (少数相の未モデル寄与)。19.25% は物理妥当性が落ちた状態である。

**基準は decision 1(a) に従い改定** — 到達値と理由をベンチ README に記録し、
**「19.3% / validity fail」を記録値として未達のまま**とした。GSAS 側の ≤15% は据え置く。
