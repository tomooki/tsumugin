...
- **箱は最適化中の制約ではない** — `dropOOBvars` は精密化**後**に「境界へ丸めて `parmFrozen`
  ...
  変数」に限定する。
- **「狭い窓で差を取る」は成立しない** (レビューで判明・当初の規定を撤回) — `dropOOBvars` は
  段の**すべての**精密化呼び出し (初回・収束サイクル・救済サイクル) で走るため窓は段全体に
  及び、狭められない。かつ救済が凍らせる「弱い変数」には箱付き変数 (`0::A0` / `:0:Shift` /
  `0:0:Size;i`) が普通に含まれるので、上の絞り込みも素通りする。**engine が自分で凍らせた名前を
  差分の基準側へ足す** (`engine._bound_hit_baseline`) 形へ改めた。
