...
- **各フレームの fit は既定で全部保存される** (2026-08-20 規定)。系列の結果に `gpx_dir`
  (run ディレクトリ) と `frames[].gpx_path` が入り、棄却された相追加トライアルと M10 の
...
- **`top_features` の + 残差** (obs > calc) = **未説明ピーク** = 欠落相の候補。2θ → d 値 →
  ...
  **波長が判らない/TOF 中性子 (単一波長なし) なら `wavelength=None` を明示的に渡す** — 省略すると
  既定 Cu Kα1 (1.5406) が使われ、異方 re-score が誤波長で走って候補の順位付けが壊れる
  (2θ のずれが照合許容 0.15° を大きく超える)。`None` は「不明」= 波長依存の段を両方止める。
  代表波長をでっち上げてはならない。
