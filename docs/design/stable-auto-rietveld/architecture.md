...
**(a) 箱を張った変数に限定** し、**(b) engine が自分で凍らせた名前を差分の基準側へ足す**
(`engine._bound_hit_baseline`)。

(b) は当初「精密化呼び出しの前後という狭い窓で差を取る」と規定していたが、**その窓は取れない**
と判明した — `dropOOBvars` は段の*すべての*精密化呼び出し (初回・収束サイクル・救済サイクル)
で走るので、窓は必ず段全体に及ぶ。かつ救済が凍らせるのは「弱い変数」であって、`0::A0` /
`:0:Shift` / `0:0:Size;i` はどれもその候補になり得る = (a) の絞り込みを素通りする。窓を狭める
方向では切り分けられないので、**帰属の判っている凍結だけを基準へ足して除く**形に改めた。
