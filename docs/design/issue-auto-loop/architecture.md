...
対象: `tomooki/tsumugin` (public, OPEN 49 件 / 2026-08-20 時点。ゲート付与時は 53 件で、
作業中に M12 の #173/#175/#178/#180 が COMPLETED でクローズされた)。
...
- **run レベルの失敗は 1 回で `loop:blocked`。回数は数えない。** 試行回数を数える仕組みが
  どこにも無いのに「2 回まで自動再試行」と宣言するのは、**実装されていないガードを宣言する**
  ことになる (この判断は「落ちないガードは無いより悪い」と同じ規律)。再開は人間が
  `loop:blocked` を外して `loop:queued` を付け直す明示的な操作とする。
  なお SKILL.md §6 の「戻れるのは 2 回まで」は**周回内**の S6 リトライであって別物である。
...
- **ラベルを剥がさない** — `loop:*` の遷移以外でラベルを消さない (優先度・領域・ゲートは
  ワーカーの判断対象外。§9.1 のルーティンも追加しかせず削除はしていない)。
- **失敗した Issue を自動で再試行しない** — 1 回失敗したら `loop:blocked`。試行回数を数える
  仕組みが無いので「N 回までは再試行」と振る舞わない。再開は人間の明示操作。
...
### 9.1 トリアージは自動化した (2026-09-03 改定)

**旧宣言「トリアージそのものは自動化しない — 優先度の判断は人間」は撤回する。**
定期実行のルーティン (クラウドエージェント) が OPEN 全件に**一次トリアージ**を付けている。
トラッカーから観測できる実体は次のとおり:

| 項目 | 実測 |
|---|---|
| 語彙 | `triaged` / `type:{bug,feature,docs,question}` / `prio:{high,medium,low}` / `duplicate?` (2026-09-02 新設) |
| 実行 | 2026-09-03 13:42 (`tomooki` 名義) → 09-03 23:22 / 09-04 09:14 (`claude[bot]` 名義) の 3 回 |
| 範囲 | OPEN 54 件すべてに `triaged` + `type:*` + `prio:*`。`duplicate?` は #211 の 1 件のみ |
| 触らないもの | `gate:*` / `loop:*` / `priority:*` / `area:*`。**ラベルの削除 (`unlabeled`) は観測範囲で 0 件** |

**§3 の状態機械は影響を受けない** — ループが読むのは `gate:*` と `loop:*` だけで、
ルーティンはそのどちらにも触れない。

#### それでも人間に残るもの

- **`gate:*` の付与** — ゲート級は難易度ではなく **§1(a) 検証の非対称性**の写像である
  (CI で証明できるか / ローカルの GSAS-II と実データが要るか / 既定を動かすのでベンチ
  再測定が要るか)。誤ると「fast tier が green だから完了」と報告する周回になる
  = §4 の無言成功そのもの。**ルーティンはゲートを付けないので、ルーティンだけを回しても
  ループのキューは 1 件も増えない** — ここが「トリアージが自動化された」ことと
  「ループが自走する」ことが**別**である理由。
- **本文とコードの食い違いの検出** — 2026-08-19 は 53 件中 5 件、うち 2 件は
  **本文どおり直すと壊れる**ものだった (#200 / #189)。これはラベルの仕事ではなく
  実地確認 (S1) の仕事であり、ルーティンの一次見立てはここを保証しない。

#### 二重語彙 — 正は `priority:*`

`priority:P0..P4` (人間) と `prio:{high,medium,low}` (ルーティン) が併存する。
**正は `priority:*`** — ゲート付与と着手順の根拠であり、実地確認を伴うため。
`prio:*` は一次見立てで、**食い違いは「見直しの合図」として読む** (捨てない・上書きしない)。

2026-08-31 時点の実測では大半が整合し、**食い違いは #211 の 1 件だけ**
(`priority:P0` ↔ `prio:medium`)。この 1 件は示唆的で、#211 は本文だけ読むと
「ADP 凍結の語彙が無い」という語彙追加の要望に見えるが、コードに当てると **#189 と
同一の 1 行の欠陥** (`engine.py:850`) だった。**食い違いは「本文だけ読んだ結論」と
「コードに当てた結論」の差として現れる**。
なお同じルーティンが #211 に `duplicate?` を独立に付けており、重複という判定自体は一致した。
...
> **撤回するときも同じ作法で書き換える** — 宣言と実態がずれたまま残った文書は、
> 次に読む者に「まだ自動化していない」と誤って教える (手順書の誤りは実装バグと同等に有害)。
