...
**位置づけの改訂 (2026-07-25)**: ハンドオフのプロトタイプは**デザイン参考** (トークン・
見た目の骨格) であり機能仕様の正ではない。機能・画面は「通常の Rietveld 解析アプリとして
必要か」を正として本書 + V2_PLAN で定める。

- **REQ-GUI-015 プロジェクト管理**: GUI からの新規作成 (名前+保存先)・開く・閉じる・
  最近使ったプロジェクト。プロジェクト = ディレクトリ + project.json (自動保存) +
  永続 ledger/snapshot (再起動後も監査履歴が続く)。
- **REQ-GUI-016 ファイル読み込み**: データ (XRDML/XY/XYE/FXYE/GSAS)・装置パラメータ・
  CIF を GUI から取り込み (プロジェクト内へコピーする自己完結方式)。ヒストグラム/相の
  追加・除去・精密化設定の編集 (全て ledger 記録)。
- **REQ-GUI-017 Welcome 画面**: プロジェクト未読込 (source=none) では作成/開く/サンプル
  の導線を表示する。
