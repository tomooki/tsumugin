...
- REQ-006: システムは TOF の立上り alpha と減衰の支配項 beta-0 が正 (> 0) か判定しなければならない
  (式中 `1/alpha`, `1/beta` でゼロ近傍発散するため)。beta-1 は d 依存の補正係数で単独では正の制約を
  持たない (小さな負値も物理的にありうる) ため hard 判定から除外する 🔵 *ユーザ入力設計(2)/T4 教訓 + 物理的精緻化*
