...
> **⚠ 実装中の重要な経験的発見 (GSAS getFWHM 精読 + T1 実測)**: CW プロファイル係数の**符号は
> revert 基準にならない**。GSAS-II `getFWHM` はガウス分散を `sqrt(max(0.001, U·tan²θ+V·tanθ+W))` と
> **0.001 でクランプ**して計算するため、U,V,W が負で H_G² が負に振れても非物理でなく、実 T1
> fluoroapatite の**良好フィットは U=-1.96, Y=-3.13 に収束**する。ローレンツ負値も反射位置以外では
> 総 FWHM に影響しない。よって当初想定の「幅関数正値性 hard 判定」は良好フィットを誤 revert する。
> hard は**真の発散のみ**に限定する (下記 REQ-002〜006 を改訂)。TOF は `sigTOF` がクランプしない・
> `1/α`,`1/β` 発散があるため係数制約が有効。
...
- REQ-002 (改訂): システムは**全放射源共通で**、解放済 (refined=True) パラメータの値が NaN/inf (真の
  発散) のとき hard (revert) と判定しなければならない 🔵 *GSAS 挙動 + 発散の定義*
- REQ-003 (改訂): システムは CW ガウス幅 `H_G²` の負値・ローレンツ X,Y の負値・SH/L の負値/過大を
  **soft 警告** (revert しない) として `ValidityReport.warnings` に記録しなければならない。GSAS がガウスを
  クランプし、係数符号が良好フィットでも負になりうるため revert 基準にしない 🔵 *getFWHM 精読/T1 実測*
- REQ-004: システムは TOF ガウス分散 `σ² = sig-0 + sig-1·d² + sig-2·d⁴` が d レンジ全域で非負 (≥ -tol) か
  判定し、sig-* 解放済のとき違反を hard としなければならない (GSAS `sigTOF` は sqrt をクランプせず NaN 化) 🔵 *getFWHM 精読*
- REQ-005: システムは TOF の立上り alpha と減衰の支配項 beta-0 が正 (> 0) か判定し、解放済のとき違反を
  hard としなければならない (式中 `1/alpha`, `1/beta` 発散)。beta-1 は d 依存補正係数で hard 除外 🔵 *getFWHM 精読/T4 教訓*
- REQ-006: システムは判定用に CW ガウス幅二乗のレンジ区間最小 (端点 + 頂点 `tanθ*=-V/2U`) と TOF 分散の
  レンジ区間最小 (端点 + 頂点) を計算しなければならない (soft 警告 / TOF hard の根拠) 🔵 *2次関数区間最小*

### 条件付き要件

- REQ-101: プロファイル hard 違反 (NaN/inf・TOF σ²<0・TOF alpha/beta-0≤0) が検出された場合、システムは
  当該段階を非物理とみなし `rwp = gof = inf` に変換して、既存の revert 経路 (`engine.py:635`) に処理させ
  なければならない 🔵 *ユーザ入力設計(3)*
- REQ-102: あるパラメータが**解放されていない (refined=False)** 場合、その hard 判定 (符号/発散) を
  revert 判定に用いてはならない (初期 instprm の borderline 値による誤 revert を防ぐ) 🔵 *ユーザ入力設計(2) 誤revert防止*
- REQ-103: soft (CW 係数の符号・過大な SH/L 等) の場合、システムは revert せず `ValidityReport.warnings`
  に記録しなければならない 🔵 *ユーザ入力設計(4) 第2層*
