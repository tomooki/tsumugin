...
1. Dara互換ベンチ(前駆体混合+固相反応生成物、Dara/Jade/人間比較)。
2. Operandoベンチ — グラファイトステージング、LFP二相、P2型固溶体域+OP4転移。FR-313/316の判別・分割正答性、FR-317吸収補正の有無による相分率バイアス評価。
3. 高温ベンチ — 既知反応系での転移温度・相系譜再現。
4. 較正ベンチ — reliability diagram / ECE。`bic` と `nested` の確率較正を別々に評価。
5. jointベンチ — X線+中性子併用時の占有率精度向上の定量(Ni/Fe/Mn系を想定)。
