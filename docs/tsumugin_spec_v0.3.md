...
- NFR-108: **精密化成果物の全保存** (2026-08-20 追加) — 実際に精密化バックエンドを起動した単位
  ごとに成果物 (GSAS `.gpx` / TOPAS プロジェクト) を 1 つ保存し、そのパスを結果に含めねばならない。
  対象は単一パターン精密化に限らず、**系列の各フレーム・棄却された相追加トライアル・双方向
  (前方/後方) パス・マルチスタートの各開始点・レシピ探索の各候補・モデル比較の各バリアント**を
  含む。保存を無効化できること (opt-out) を要するが、**既定は保存**とする。
  根拠: 受理/棄却の判断 (FR-118-7, FR-337 の bic crossover, FR-330 の受理基準) は数値要約に
  畳まれるため、保存しない限り**判断そのものを事後検証できない**。また MEM (FR-601/603) は
  精密化済み成果物を入力に要求する。保存先・命名・索引は実装の裁量とするが、系列は 1 実行分を
  1 か所にまとめ、索引 (役割・相・Rwp) を追記型で残すこと (P2/NFR-105 と整合)。
