...
補正は**既知相を引いた残差に対して**行う (少数相を生パターンへ整合させると FoM が支配相の
ピークに占められセルが悪化するため)。既知相を引けないときは補正せず DFT 格子のまま渡す。
`identify_and_add_phase` を手で呼ぶときは**現行相を `known_phases` に渡す** — 渡さないと
補正が入らず (`prealign_basis="skipped"`)、DFT 格子のままの CIF が返る。
