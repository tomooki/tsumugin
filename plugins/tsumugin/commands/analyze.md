---
description: 粉末回折データを全自動 Rietveld 解析する (tsumugin agentic 閉ループ)。データ/装置/相ファイルを指定すると、MCP 3 ツールを反復駆動して Rwp/GOF と物理妥当性を最適化する。SafeAction は自律、構造改訂・相追加はユーザー承認を挟む。
...
3. MCP ツール `auto_rietveld` → `propose_next_actions` → `refine_with_revisions` を反復駆動する。
