...
- **失敗したら 1 回で `loop:blocked` + 理由コメント。自動で再試行しない。**
  試行回数を数える仕組みは無いので、「N 回までは自動再試行」と振る舞ってはならない
  (数えられないものを数えたことにするのが、失敗が伝わらないまま回り続ける最悪の形)。
  再開は人間が `loop:blocked` を外して `loop:queued` を付け直す明示的な操作。
  ※ §6 の「戻れるのは 2 回まで」は**この周回の中**の S6 リトライであって別物。
